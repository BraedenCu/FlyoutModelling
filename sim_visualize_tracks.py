...
class TrajectoryVisualizer:
    ...
    def setup_topography(self, bounds: Tuple[float, float, float, float], satellite_path: str = None):
        ...
        if self.topography_manager.satellite_manager.satellite_texture is not None:
            ...
            # Get satellite image bounds for proper texture mapping
            satellite_bounds = self.topography_manager.satellite_manager.satellite_bounds
            if satellite_bounds is not None:
                # Use satellite image bounds for texture coordinates
                sat_left, sat_bottom, sat_right, sat_top = satellite_bounds
                print(f"Using satellite bounds for texture mapping: {satellite_bounds}")
                
                # Calculate texture coordinates based on satellite image bounds
                u = (X - sat_left) / (sat_right - sat_left)
                v = (Y - sat_bottom) / (sat_top - sat_bottom)
                ...
                # Set texture coordinates on surface
                surface.active_texture_coordinates = tex_coords.astype(np.float32)
                print(f"Set texture coordinates on topography surface using satellite bounds")
                print(f"Texture coordinates shape: {tex_coords.shape}")
                print(f"Texture coordinates range: {tex_coords.min()} to {tex_coords.max()}")
            else:
                print("Warning: Satellite bounds not available, using fallback texture coordinates")
    ...
    def add_topography_to_plot(self):
        ...
        if self.topography_manager.satellite_manager.satellite_texture is not None:
            ...
            # Enable lighting to show 3D topographic features
            self.plotter.add_mesh(
                self.topography_mesh,
                texture=self.topography_manager.satellite_manager.satellite_texture,
                show_edges=False,
                lighting=True,  # Enable lighting to show 3D features
                ambient=0.4,    # Increase ambient lighting for better texture visibility
                diffuse=0.6,
                specular=0.2,
                specular_power=15,
                smooth_shading=True,
